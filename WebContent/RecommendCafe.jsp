            ...
            <div id="title">
				...
				<header id="header">
						...
						<div class="logo" style="padding-top: 1.1rem">
							<a href=#cafe><img src="images/pinfinal.png" style="width:50%; height:70%"></a>
				...
				<!-- Main -->
					...
					<!-- Contact -->	
						 ...
						 <!-- recmommend 추천 -->
		 					...
		 					<% /* positionVO pvo = (positionVO)session.getAttribute("pvo"); */
								...
								ArrayList<cafeVO> cafe_voArr = (ArrayList<cafeVO>)session.getAttribute("cafe_voArr");
						     	System.out.println("jsp = " + cafe_voArr.get(0).getName());
						     	System.out.println("jsp = " + cafe_voArr.get(0).getLatitude());
						     	System.out.println("jsp = " + cafe_voArr.get(0).getLongitude());
						...
						<script type="text/javascript">
							function move(i) {
								/* var index = i */
								var check = confirm("최종 선택 하시겠습니까?")
								if(check==true){
									location.href="ResultCon?cafe_id="+i;	
								}else{
									
									location.href="RecommendCafe.jsp#cafe"
								}
						...
						<style>
					         ...
					         <article id="cafe" style="padding-top: 1.3rem">
					          <input type="button" value="코스선택" style="margin-bottom: 1rem;" onclick="location.href='RecommendCourse.jsp#course'">
					         	<div id="map">
					         	</div>
								    ...
								    mapOption = { 
								        center: new kakao.maps.LatLng(<%= cafe_voArr.get(0).getLatitude()%>, <%= cafe_voArr.get(0).getLongitude()%>), // 지도의 중심좌표
								        level: 9 // 지도의 확대 레벨
								    };
									...
									var positions = [
										<%for(int i=0; i<cafe_voArr.size();i++){%>
										 {	
										     content: "<div style='width:400px; height:400px; display:block; text-align:center;'><%= cafe_voArr.get(i).getName() %><img id='cafeImage' src='images/cafe_pic/<%= cafe_voArr.get(i).getName() %>.JPG'><table id='cafe_info'><tr><td>카페 주소</td><td><%= cafe_voArr.get(i).getAddress() %></td></tr></table><div style='text-align:center;'><input id='cafeserch' type='button' value='결  정!' onclick='move(<%= i %>)'></div></div>", 
										     latlng: new kakao.maps.LatLng(<%= cafe_voArr.get(i).getLatitude()%>, <%= cafe_voArr.get(i).getLongitude()%>)  
